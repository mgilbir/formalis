...
func TestCIUSPTCorpus(t *testing.T) {
	...
	for _, f := range files {
		...
		if pt := ptRuleViolations(ValidateCIUSPT(data)); len(pt) != 0 {
...
func TestCIUSPTMutations(t *testing.T) {
	if pt := ptRuleViolations(ValidateCIUSPT([]byte(minimalCIUSPTUBL))); len(pt) != 0 {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !hasFacturXRule(ValidateCIUSPT([]byte(broken)), tc.want) {
				t.Errorf("expected %s to fire; got %v", tc.want, ptRuleViolations(ValidateCIUSPT([]byte(broken))))
			}
		})
	}
}
