...
func TestEbInterfaceCorpus(t *testing.T) {
	...
	for _, f := range files {
		...
		if v := ValidateEbInterface(data); len(v) != 0 {
...
func TestEbInterfaceMutations(t *testing.T) {
	if v := ValidateEbInterface([]byte(minimalEbInterface)); len(v) != 0 {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !hasFacturXRule(ValidateEbInterface([]byte(broken)), tc.want) {
				t.Errorf("expected %s to fire; got %v", tc.want, ValidateEbInterface([]byte(broken)))
			}
		})
	}
}
