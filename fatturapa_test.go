...
func TestFatturaPACorpus(t *testing.T) {
	...
	for _, f := range files {
		...
		if v := findings(t, context.Background(), ValidateFatturaPA, data); len(v) != 0 {
...
func TestFatturaPAMutations(t *testing.T) {
	if v := findings(t, context.Background(), ValidateFatturaPA, []byte(minimalFatturaPA)); len(v) != 0 {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !hasFacturXRule(findings(t, context.Background(), ValidateFatturaPA, []byte(broken)), tc.want) {
				t.Errorf("expected %s to fire; got %v", tc.want, findings(t, context.Background(), ValidateFatturaPA, []byte(broken)))
			}
		})
	}
}
