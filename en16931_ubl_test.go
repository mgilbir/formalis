...
func TestValidateUBLMutations(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			v := findings(t, context.Background(), withProfile(ProfileEN16931), []byte(broken))
			found := false
			for _, x := range v {
...
func TestTaxTotalSelectedByCurrency(t *testing.T) {
	...
	for _, tc := range []struct{ name, xml string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			if v := findings(t, context.Background(), withProfile(ProfileEN16931), []byte(tc.xml)); len(v) != 0 {
	...
	a := findings(t, context.Background(), withProfile(ProfileEN16931), []byte(docFirst))
	b := findings(t, context.Background(), withProfile(ProfileEN16931), []byte(accFirst))
	if len(a) != len(b) {
	...
	var co15 string
	for _, v := range findings(t, context.Background(), withProfile(ProfileEN16931), []byte(brokenGrand)) {
		if v.Rule == "BR-CO-15" {
			co15 = v.Message
		}
	}
...
func TestInvoicingPeriodOrdersCalendarDays(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got := hasFacturXRule(findings(t, context.Background(), withProfile(ProfileEN16931), withPeriod("InvoicePeriod", tc.start, tc.end)), "BR-29")
	...
	if hasFacturXRule(findings(t, context.Background(), withProfile(ProfileEN16931), sameDay), "BR-30") {
	...
	if !hasFacturXRule(findings(t, context.Background(), withProfile(ProfileEN16931), outOfOrder), "BR-30") {
...
func TestVATIdentifierCountryPrefix(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.id, func(t *testing.T) {
			got := hasFacturXRule(findings(t, context.Background(), withProfile(ProfileEN16931), withSellerVATID(tc.id)), "BR-CO-09")
			if got != tc.wantViolation {
	...
	if hasFacturXRule(findings(t, context.Background(), withProfile(ProfileEN16931), noVATID), "BR-CO-09") {
...
func TestVATAmountTolerance(t *testing.T) {
	...
	if hasFacturXRule(findings(t, context.Background(), withProfile(ProfileEN16931), []byte(within)), "BR-CO-17") {
	...
	if !hasFacturXRule(findings(t, context.Background(), withProfile(ProfileEN16931), []byte(beyond)), "BR-CO-17") {
...
func TestBindingRuleIDsPerSyntax(t *testing.T) {
	...
	v := findings(t, context.Background(), withProfile(ProfileEN16931), []byte(cii))
	...
	v = findings(t, context.Background(), withProfile(ProfileEN16931), []byte(ubl))
	if !hasFacturXRule(v, "UBL-SR-47") {
...
func TestValidateUBLCalcMutation(t *testing.T) {
	...
	v := findings(t, context.Background(), withProfile(ProfileEN16931), broken)
	found := false
	for _, x := range v {
		if x.Rule == "BR-CO-15" {
			found = true
