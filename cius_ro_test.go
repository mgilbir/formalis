...
func TestCIUSROCorpus(t *testing.T) {
	...
	for _, f := range files {
		...
		if ro := roRuleViolations(ValidateCIUSRO(data)); len(ro) != 0 {
...
func TestCIUSROMutations(t *testing.T) {
	if ro := roRuleViolations(ValidateCIUSRO([]byte(minimalCIUSROUBL))); len(ro) != 0 {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !hasFacturXRule(ValidateCIUSRO([]byte(broken)), tc.want) {
				t.Errorf("expected %s to fire; got %v", tc.want, roRuleViolations(ValidateCIUSRO([]byte(broken))))
			}
		})
	}
}
