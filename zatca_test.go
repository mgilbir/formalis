...
func TestZATCACorpus(t *testing.T) {
	...
	for _, f := range files {
		...
		if v := findings(t, context.Background(), ValidateZATCA, data); len(v) != 0 {
...
func TestZATCAMutations(t *testing.T) {
	if v := findings(t, context.Background(), ValidateZATCA, []byte(minimalZATCA)); len(v) != 0 {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !hasFacturXRule(findings(t, context.Background(), ValidateZATCA, []byte(broken)), tc.want) {
				t.Errorf("expected %s to fire; got %v", tc.want, findings(t, context.Background(), ValidateZATCA, []byte(broken)))
			}
		})
	}
}
