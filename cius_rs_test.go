...
func TestSRBDTCorpus(t *testing.T) {
	...
	for _, f := range files {
		...
		if rs := rsRuleViolations(ValidateSRBDT(data)); len(rs) != 0 {
...
func TestSRBDTMutations(t *testing.T) {
	if rs := rsRuleViolations(ValidateSRBDT([]byte(minimalSRBDT))); len(rs) != 0 {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !hasFacturXRule(ValidateSRBDT([]byte(broken)), tc.want) {
				t.Errorf("expected %s to fire; got %v", tc.want, rsRuleViolations(ValidateSRBDT([]byte(broken))))
			}
		})
	}
}
