...
func TestFinvoiceCorpus(t *testing.T) {
	...
	for _, f := range files {
		...
		if v := findings(t, context.Background(), ValidateFinvoice, data); len(v) != 0 {
...
func TestFinvoiceMutations(t *testing.T) {
	if v := findings(t, context.Background(), ValidateFinvoice, []byte(minimalFinvoice)); len(v) != 0 {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !hasFacturXRule(findings(t, context.Background(), ValidateFinvoice, []byte(broken)), tc.want) {
				t.Errorf("expected %s to fire; got %v", tc.want, findings(t, context.Background(), ValidateFinvoice, []byte(broken)))
			}
		})
	}
}
