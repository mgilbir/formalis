...
func TestValidateXRechnungBaseline(t *testing.T) {
	if v := ValidateXRechnung(context.Background(), []byte(minimalXRechnungUBL)).Violations; len(v) != 0 {
  ...
  <cac:PayeeFinancialAccount><cbc:ID>DE75512108001245126199</cbc:ID></cac:PayeeFinancialAccount></cac:PaymentMeans>`, "", "BR-DE-1"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			v := ValidateXRechnung(context.Background(), []byte(broken)).Violations
			found := false
			for _, x := range v {
				if x.Rule == tc.rule {
					found = true
...
func TestValidateXRechnungCorpus(t *testing.T) {
	...
	filepath.Walk(root, func(p string, fi os.FileInfo, e error) error {
		...
		files++
		if v := ValidateXRechnung(context.Background(), data).Violations; len(v) != 0 {
